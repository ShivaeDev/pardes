...
## Leak hygiene for public artifacts
If any repo you orchestrate publishes to a public surface, the text you author is a leak vector your code scanners do not cover.
- **Public text you author is an unscanned leak surface.** Pre-commit and content hooks scan staged *diffs* — they do NOT scan PR titles, PR bodies, commit messages, or issue/PR comments, because those go through the host's CLI/API and never touch the VCS. Self-scan every public-facing string you write against your private denylist before publishing, exactly as you would scan a diff. A clean diff creates false confidence; the prose around it is wide open.
- **Never describe a public artifact by its private origin.** Describe only what the artifact *is* and *does* — never where it came from, which private project it was lifted from, or who it was built for. The genericized result must read as if it were authored for the public from scratch.
- **Scrubbing a pre-merge slip is allowed via your own branch.** If a private detail lands on an unmerged feature branch you own, force-push the corrected branch (`--force-with-lease`) to scrub it — that is cleaner than close-and-recreate, and the no-force rule targets *shared / merged* history, not your own pre-merge branch. **Exception:** if the repo's own config mandates an absolute no-force policy, honor it — fix-forward and ask the maintainer how they want the slip handled.

## Workflow library
Workflows can be saved and reused. Build a library over time — the highest-value pattern is **writer → reviewer**: one agent implements a PR in a worktree, a second agent adversarially reviews the diff (or posts inline review comments) before the PR opens. Start ad hoc; promote the ones that prove useful into saved workflows. Record orchestration learnings in the `LEARNINGS.md` beside this skill — it ships seeded with the operational lessons this skill is built on, and you append to it as you learn, so future shift-leader sessions inherit them. If that location is not writable, fall back to project memory or a tracked notes file.