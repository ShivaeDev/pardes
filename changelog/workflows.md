...
- `writer-reviewer` — implement a change in a worktree-isolated agent, then run an adversarial reviewer over the diff (bounded fix⟲re-review loop). Never merges or opens a PR; returns the diff location and a structured verdict.
...
- `parallel-verify` — fan out one worktree-isolated writer per disjoint, caller-supplied partition, then run a single serialized heavy verification (parallel heavy runs exhaust memory). Never merges or opens PRs; returns per-partition results plus the verification outcome.
